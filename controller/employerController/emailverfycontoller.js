...
const sendOtpToEmail = async (req, res) => {
  ...
  try {
    ...
    // Store OTP in app.locals (in-memory)
    req.app.locals.otps = req.app.locals.otps || {}; // ensure object exists
...
const sendOtpToEmailEmployer = async (req, res) => {
  const { userEmail } = req.body;
  console.log("check")

  try {
    const employer = await EmployerSchema.findOne({ userEmail });
    if (employer) {
      return res
        .status(401)
...
const verifyEmailOtpEmployer = async (req, res) => {
  const { userEmail, otp } = req.body;
  console.log("userEmail, otp", userEmail, otp);
...
const sendOtpToEmailEmployerAdmin = async (req, res) => {
  ...
  try {
    const employer = await employerAdminSchema.findOne({ userEmail });
    if (employer) {
      return res
        .status(401)
        .json({ message: "Employer email is already registered." });
...
const sendForgotOtpToEmailEmployer = async (req, res) => {
  const { userEmail } = req.body;
  console.log("check")
  ...
  try {
    ...
    if (!employer) {
      return res
        .status(401)
        .json({ message: "Employer email is not registered." });
    }
...
const verifyEmailForgotOtpEmployer = async (req, res) => {
  const { userEmail, otp } = req.body;
  console.log("userEmail, otp", userEmail, otp);
...
module.exports = {
  verifyEmailForgotOtpEmployer,
  sendForgotOtpToEmailEmployer,
  sendOtpToEmail,
  verifyEmailOtp,
  verifyEmailOtpEmployer,
  sendOtpToEmailEmployer,
